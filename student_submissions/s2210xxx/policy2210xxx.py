...
class Policy2210xxx(Policy):
    ...
    def get_action(self, observation, info):
        ...
        rectangles = [Rectangle(prod["size"][0], prod["size"][1], prod["quantity"]) for prod in list_prods if prod["quantity"] > 0]
        # Sắp xếp các hình chữ nhật theo diện tích giảm dần
        rectangles.sort(key=lambda x: x.width * x.height, reverse=True)

        stock_areas = [(i, self._get_stock_size_(stock)[0] * self._get_stock_size_(stock)[0]) for i, stock in enumerate(observation["stocks"])]
