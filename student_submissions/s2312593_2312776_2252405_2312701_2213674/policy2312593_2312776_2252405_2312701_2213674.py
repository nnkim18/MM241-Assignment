...
class Policy2312593_2312776_2252405_2312701_2213674(Policy):
...
class ModifiedGreedy(Policy):
    ...
    def evaluate(self):
        ...
        print(" - Waste Percent:  ",
              (1-self.m_filled_surface/self.m_used_surface)*100, "%")
    ...
    def init_indices(self, list_stocks, list_prods):
        ...
        sorted_products = sorted(
            list_prods, key=lambda product: product['size'][0] * product['size'][1], reverse=True)
        product_indies = []
        ...
        sorted_stocks = sorted(list_stocks, key=lambda stock: np.sum(np.any(
            stock != -2, axis=1)) * np.sum(np.any(stock != -2, axis=0)), reverse=True)
        stock_indies = []
...
class FirstFitDecreasingHeuristic(Policy):
                            ...
                            for x in range(stock_w - prod_w + 1):
                                for y in range(stock_h - prod_h + 1):
                                    if self._can_place_(stock, (x, y), prod_size):
                                        pos_x, pos_y = x, y
                                        self.paint(st_idx, pr_idx,
                                                   (pos_x, pos_y), prod_size)
                                        # thêm bước thêm action vào 1 danh sách
                                        self.action_list[st_idx].append({"stock_idx": st_idx, "size": prod_size, "position": (
                                            pos_x, pos_y), "product_idx": pr_idx})
                                        break
                                if pos_x is not None and pos_y is not None:
                                    break
                            ...
                            for x in range(stock_w - prod_h + 1):
                                for y in range(stock_h - prod_w + 1):
                                    if self._can_place_(stock, (x, y), new_size):
                                        pos_x, pos_y = x, y
                                        self.paint(st_idx, pr_idx,
                                                   (pos_x, pos_y), new_size)
                                        # thêm bước thêm action vào 1 danh sách
                                        self.action_list[st_idx].append({"stock_idx": st_idx, "size": new_size, "position": (
                                            pos_x, pos_y), "product_idx": pr_idx})
                                        break
                                if pos_x is not None and pos_y is not None:
                                    break
            ...
            for ele in reversed(sorted_list):
                ...
                for st_idx2 in reversed(self.stocks_indices):
                    ...
                    if self._can_place_(check_stock, (0, 0), (temp_w, temp_h)):
                        self.copyAtoB(st_idx, (0, 0), st_idx2,
                                      (0, 0), (temp_w, temp_h), False)
                        break
    ...
    def copyAtoB(self, idxA, posA, idxB, posB, size, rotate):
        ...
        for i in range(width):
            for j in range(height):
                self.stocks[idxB][x_B+i][y_B +
                                         j] = self.stocks[idxA][x_A+i][y_A+j]
    ...
    def init_variable(self, list_stocks, list_products):
        ...
        self.cutted_stocks = np.full(
            (self.num_stocks,), fill_value=0, dtype=int)

        sorted_products = sorted(
            self.products, key=lambda product: product['size'][0] * product['size'][1], reverse=True)
        product_indies = []
        for s_st in range(len(sorted_products)):
        ...
        sorted_stocks = sorted(self.stocks, key=lambda stock: np.sum(np.any(
            stock != -2, axis=1)) * np.sum(np.any(stock != -2, axis=0)), reverse=True)
        stock_indies = []
        for s_st in range(len(sorted_stocks)):
    ...
    def get_action_from_list(self):
        # lấy Action
        flattened_action_list = [
            action for sublist in self.action_list for action in sublist]
    ...
    def evaluate(self):
        ...
        # hiển thị
        print(
            "[----------==========| EVALUATE FIRST FIT DECREASING HEURISTIC |==========----------]")
...
class FirstFitDecreasing(Policy):
    ...
    def evaluate(self):
        print(
            "[----------==========| EVALUATE BASIC FIRST FIT DECREASING |==========----------]")
        print(" - Stocks used:    ", self.m_used_stock)
        print(" - Used Surface:   ", self.m_used_surface)
        print(" - Waste Surface:  ", self.m_used_surface - self.m_filled_surface)
        print(" - Filled Surface: ", self.m_filled_surface)
