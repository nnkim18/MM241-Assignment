...
class Policy2311388_2312613_2312271_2313145(Policy):
...
class FirstFitDecreasing(Policy):
    ...
    def max_prod(self, list_prods, mode=2):
        if mode == 0:
            list_prods = sorted(list_prods, reverse=True,
...
class BestFitDecreasing(Policy):
    ...
    def get_action(self, observation, info):
        ...
        list_prods.sort(key=lambda x: x["size"]
                        [0] * x["size"][1], reverse=True)
            ...
            if prod["quantity"] > 0:
                ...
                if num_products < 4:
                    while prod["quantity"] > 0:
                        smallest_stock_idx = self._find_smallest_stock_(
                            used_stocks)
                        while smallest_stock_idx is not None:
                            pos_x, pos_y = self._dp_cutting_(
                                self.stocks_list[smallest_stock_idx], prod_size)
                            if pos_x is None and pos_y is None:
                                # Try rotating the product 90 degrees
                                rotated_size = (prod_size[1], prod_size[0])
                                pos_x, pos_y = self._dp_cutting_(
                                    self.stocks_list[smallest_stock_idx], rotated_size)
                                if pos_x is not None and pos_y is not None:
                                    prod_size = rotated_size

                            if pos_x is not None and pos_y is not None:
                                self._update_stock_(
                                    observation, smallest_stock_idx, prod_size, pos_x, pos_y)
                                prod["quantity"] -= 1
                                ...
                                smallest_stock_idx = self._find_next_smallest_stock_(
                                    smallest_stock_idx, used_stocks)
                ...
                if best_stock_idx != -1:
                    self._update_stock_(
                        observation, best_stock_idx, prod_size, best_pos_x, best_pos_y)
                    prod["quantity"] -= 1
                    return {
    ...
    def _evaluate_stock_(self, stock, prod_size):
        ...
        # Prioritize the smallest part
        waste_factor = (stock_w - prod_w) + (stock_h - prod_h)
    ...
    def _calculate_leftover_scores(self, leftovers):
        ...
        for leftover in leftovers:
            # Calculate the area of the leftover
            score = leftover["size"][0] * leftover["size"][1]
            scores.append(score)
        return scores
    ...
    def _update_stock_(self, observation, stock_idx, prod_size, pos_x, pos_y):
        ...
        self.stocks_list[stock_idx][pos_x:pos_x +
                                    prod_w, pos_y:pos_y + prod_h] = -2
    ...
    def _find_smallest_stock_(self, used_stocks):
        ...
        for i, stock in enumerate(self.stocks_list):
            if i in used_stocks:
                continue
            stock_size = self._get_stock_size_(
                stock)[0] * self._get_stock_size_(stock)[1]
    ...
    def _find_next_smallest_stock_(self, current_idx, used_stocks):
        ...
        current_stock_size = self._get_stock_size_(self.stocks_list[current_idx])[
            0] * self._get_stock_size_(self.stocks_list[current_idx])[1]
        for i, stock in enumerate(self.stocks_list):
