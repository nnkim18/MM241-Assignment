...
class Policy2353017_2353215_2352030_2352588(Policy):
    ...
    def get_action(self, observation, info):
        if self.policy_id == 1:
            # BOTTOM LEFT DECREASING (BLD) ALGORITHM
    ...
    def get_action1(self, observation, info):
        ...
        if self.list_prods is None:
            self.list_prods = tuple(sorted(observation["products"], key=lambda x: (
                x["size"][0] * x["size"][1], max(x["size"][1], x["size"][0]), min(x["size"][1], x["size"][0])), reverse=True))
            # area                           #height        #width
        prod_size = [0, 0]
        stock_idx = -1
        pos_x, pos_y = 0, 0
    ...
    def get_action2(self, observation, info):
        ...
        if self.list_prods is None:
            self.list_prods = tuple(sorted(observation["products"], key=lambda x: (
                max(x["size"][1], x["size"][0]), min(x["size"][1], x["size"][0])), reverse=True))
            # height        #width
                        ...
                        for x in range(stock_w - prod_size[0] + 1):
                            ...
                            if self._can_place_(stock, (x, y), size_to_check):
                                ...
                                if self.first_place[i] == 1:
                                    ...
                                    self.level_limit[i][1] = self.level_limit[i][0] + \
                                        prod_size[1] - 1
                                break
                        if pos_x is not None and pos_y is not None:
                            break
    ...
    def get_action3(self, observation, info):
        ...
        if self.list_prods is None:
            self.list_prods = tuple(sorted(observation["products"], key=lambda x: (
                max(x["size"][1], x["size"][0])), reverse=True))
            # height        #width
        for i, stock in enumerate(observation["stocks"]):
            stock_w, stock_h = self._get_stock_size_(stock)

        if self.level_limit is None:
            self.level_limit = np.empty(
                (len(observation["stocks"])), dtype=object)
            for i, stock in enumerate(observation["stocks"]):
                stock_w, stock_h = self._get_stock_size_(stock)
                self.level_limit[i] = []
