...
class Policy2311186_2310853_2310974_2311063_2310965(Policy):
    ...
    def get_action(self, observation, info):
        ...
        if self.mode == "greedy":
            if not self.initialized:
                ...
                self.total_products = sum(prod["quantity"]
            ...
            if self.current_action_idx < len(self.placements):
                ...
                self.current_action_idx += 1
                total_remaining_products = sum(
                    prod["quantity"] for prod in observation["products"])
                print(f"-------------------")
        ...
        # Code cho Genetic Algorithm
            ...
            if not self.solution:
                ...
                self.stocks_size = [
                    self._get_stock_size_(s) for s in self.stocks]
                self.products = observation["products"]
                self.products_size = [p["size"]
                                      for p in self.products for _ in range(p["quantity"])]
                # Tùy chọn kích thước quần thể và số thế hệ tối đa
                self.max_gen = MAX_GEN
                self.pop_size = POP_SIZE
                print(
                    f"Numb of products: {self.products_size.__len__()} and numb of stocks: {self.stocks.__len__()}")
    ...
    def prepare_placements(self, observation):
        ...
        stocks_with_index = [(i, stock.copy())
                             for i, stock in enumerate(observation["stocks"])]

        # Sắp xếp theo tiêu chí và giữ index gốc
        stocks_with_index = sorted(
            stocks_with_index,
            key=lambda item: np.sum(
                np.any(item[1] != -2, axis=1) * np.any(item[1] != -2, axis=0)),
            reverse=True
        )
        ...
        for stock_idx, stock in enumerate(self.stocks):
            for prod in self.list_prods:
                while prod["quantity"] > 0:
                    empty_regions = self._find_edge_regions(
                        stock, prod["size"])
                    ...
                    for x, y in empty_regions:
                        if self._can_place_(stock, (x, y), prod["size"]):
                            print(
                                f"Đặt sản phẩm {prod['size']} vào vị trí ({x}, {y}) trong kho {self.stock_indices[stock_idx]}")
                            self.placements.append(
                                {
                                    # Dùng index gốc
                                    "stock_idx": self.stock_indices[stock_idx],
                                    "size": prod["size"],
                                    "position": (x, y),
                                }
                            )
                ...
                while prod["quantity"] > 0:
                    ...
                    for x, y in empty_regions:
                        if self._can_place_(stock, (x, y), rotated_size):
                            print(
                                f"Đã xoay và đặt sản phẩm {rotated_size} vào vị trí ({x}, {y}) trong kho {self.stock_indices[stock_idx]}")
                            self.placements.append(
                                {
                                    # Dùng index gốc
                                    "stock_idx": self.stock_indices[stock_idx],
    ...
    def solve(self):
        ...
        self.population_fitness = [self.fitness(
            particle) for particle in self.population]  # Đánh giá fitness của quần thể
        self.best_particle_index = np.argmax(
            self.population_fitness)  # Chỉ số của cá thể tốt nhất
    ...
    def _decode_(self, particle):
        ...
        for i, prod in enumerate(particle):
            ...
            position = self._find_best_position(
                temp_stocks[stock_idx], stock_idx, prod_size)
            if position is None:
                print(f"Product {i} cannot be placed in stock {stock_idx}")

            temp_stocks[stock_idx] = self._put_in_stock(
                temp_stocks[stock_idx], position, prod_size)
    ...
    def init_population(self, pop_size):
        ...
        while len(self.population) < pop_size:
            ...
            temp_stocks = [s.copy() for s in self.stocks]
            # (index, product_size)
            temp_products_size = list(enumerate(self.products_size))
        ...
        if len(nearby_particles) < k:
            ...
            additional_particles = rd.sample(
                [i for i, _ in enumerate(
                    self.population) if i not in nearby_particles],
    ...
    def tournament_selection(self, population, fitness_values, k=3):
        ...
        # Chỉ số của cá thể tốt nhất
        best_candidate = max(candidates, key=lambda idx: fitness_values[idx])
    ...
    def mutate(self, particle, M=0.01):
        ...
        if rd.random() < 0.8:
            ...
            particle = [(stock_des, p[1]) if p[0] ==
                        stock_src else p for p in particle]
    ...
    def crossover(self, parent1, parent2):
        ...
        if cross_type == "single":
            ...
            offspring1 = parent1[:cross_point1] + \
                parent2[cross_point1:cross_point2] + parent1[cross_point2:]
            offspring2 = parent2[:cross_point1] + \
                parent1[cross_point1:cross_point2] + parent2[cross_point2:]
        ...
        for gen in range(max_gen):
            ...
            while next_population.__len__() < self.population.__len__() - ELITE:
                ...
                if LOCAL == True and gen > EXPLORE_GEN and rd.random() < LOCAL_PROB:
                    nearby_particles_indices = self._select_local_(
                        self.population[self.best_particle_index], distance_threshold=DIS_THRESHOLD, k=2*TOURM_SIZE)
                else:
                    nearby_particles_indices = range(self.population.__len__())
                # Chọn cha mẹ bằng Tournament Selection
                parent1_indices = self.tournament_selection(nearby_particles_indices, [
                                                            self.population_fitness[p] for p in nearby_particles_indices], k=TOURM_SIZE)
                parent2_indices = self.tournament_selection(nearby_particles_indices, [
                                                            self.population_fitness[p] for p in nearby_particles_indices], k=TOURM_SIZE)
            ...
            next_population_fitness = [self.fitness(
                particle) for particle in next_population]
            # Duy trì đúng số lượng cá thể mới
            next_population_filtered = sorted(zip(next_population, next_population_fitness), key=lambda x: x[1], reverse=True)[
                :self.population.__len__() - ELITE]
            next_population, next_population_fitness = zip(
                *next_population_filtered)
            next_population = list(next_population)
            next_population_fitness = list(next_population_fitness)

            # Duy trì những cá thể tốt nhất
            elites = sorted(zip(self.population, self.population_fitness),
                            key=lambda x: x[1], reverse=True)[:ELITE]
