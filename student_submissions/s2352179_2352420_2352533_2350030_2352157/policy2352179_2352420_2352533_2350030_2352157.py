...
class GeneticAlgorithmPolicy(Policy):
    ...
    def initialize_population(self, maxRepeatArr):
        sorted_indices = np.argsort(-np.array(self.lengthArr)
                                    * np.array(self.widthArr))
        return [
    ...
    def max_pattern_exist(self, patterns):
        return [
            max(ceil(self.demandArr[i] / pattern[i])
                for i in range(len(pattern)) if pattern[i] > 0)
    ...
    def run_genetic_algorithm(self, patterns, population, max_repeat_arr):
        ...
        for _ in range(self.MAX_ITER):
            ...
            new_population = deepcopy(
                [sc[0] for sc in fitness_pairs[: self.elite_size]])
    ...
    def create_new_pop(self, population):
        return [
            self.mutate(self.crossover(*sample(population, 2)),
                        self.mutation_rate)
            for _ in range(self.pop_size)
        ]
    ...
    def get_action(self, observation, info):
        ...
        self.lengthArr = [prod["size"][0]
                          for prod in list_prods if prod["quantity"] > 0]
        self.widthArr = [prod["size"][1]
                         for prod in list_prods if prod["quantity"] > 0]
        self.demandArr = [prod["quantity"]
                          for prod in list_prods if prod["quantity"] > 0]
                ...
                for x in range(stock_Width):
                    for y in range(stock_Length):
                        if pattern_index >= len(self.lengthArr):
                            continue
                        prod_size = (
                            self.lengthArr[pattern_index], self.widthArr[pattern_index])
