...
class Policy2353269_2353235_2252723_2352689_2352608(Policy):
    ...
    def _get_action_policy2(self, observation, info):
        ...
        optimal_choice = {"stock_idx": -1,
                          "size": (0, 0), "position": (None, None)}
        best_leftover_space = float('inf')

        for product in available_products:
            candidate_placement, candidate_score = self._find_optimal_placement_for_product(
                product, stocks, best_leftover_space)
            ...
            for (try_w, try_h) in orientations:
                if try_w <= stock_w and try_h <= stock_h:
                    candidates = self._get_all_candidates(
                        stock, s_idx, try_w, try_h, stock_w, stock_h)
...
# METHOD FOR POLICY 1
    ...
    def _find_valid_positions(self, stock, prod_size):
        ...
        corners = [(0, 0), (0, stock_w-prod_w), (stock_h-prod_h, 0),
                   (stock_h-prod_h, stock_w-prod_w)]
        for x, y in corners:
    ...
    def _find_best_position(self, stock, size):
        ...
        for pos in positions:
            ...
            neighbors = 0
            if x > 0 and np.any(stock[x-1, y:y+w] != -1):
                neighbors += 1
            if x + h < stock_h and np.any(stock[x+h, y:y+w] != -1):
                neighbors += 1
            if y > 0 and np.any(stock[x:x+h, y-1] != -1):
                neighbors += 1
            if y + w < stock_w and np.any(stock[x:x+h, y+w] != -1):
                neighbors += 1
            score += neighbors * 300
