...
class Policy2313132_2313034_2314041_2311161(Policy):
...
# ----------------------------------- GENETIC ALGORITHM -----------------------------------
        ...
        for prod in products:
            ...
            for _ in range(num_prod):
                ...
                for _ in range(100):  # Limit attempts to prevent infinite loop
                    ...
                    # and (stock_w < prod_h or stock_h < prod_w):
                    ...
                    if self._can_place_(stock, (pos_x, pos_y), (prod_w, prod_h)):
                        cut_pattern = {
                            "stock_idx": stock_idx, "size": prod["size"], "position": (pos_x, pos_y)}
                        solution.append(cut_pattern)
                        self._update_stock_usage_genetic(
                            stocks_copy, cut_pattern)
    ...
    def cal_fitness(self, population, observation):
        ...
        for individual in population:
            ...
            placed_products = 0
            total_products = sum(prod["quantity"]
            ...
            for stock in stocks_copy:
                ...
                if (placed == True):
                    ...
                    total_stock_used += 1
            # Calculate fitness
            total_wasted_space = sum(np.prod(self._get_stock_size_(
                stock)) for stock in observation["stocks"])
            normalized_wasted_space = wasted_space / \
                total_wasted_space if total_wasted_space > 0 else 0
            fitness = - placed_products * 100 + 0.001 * \
                normalized_wasted_space + total_stock_used
    ...
    def mutation(self, individual, observation):
        ...
        for i in range(len(mutated)):
            if random.random() < self.mutation_rate:
                ...
                self._remove_product_from_stock(
                    stocks_copy[old_stock_idx], old_position, prod_size)

                placement_found = False
                for _ in range(100):
                    stock_idx = random.randint(0, len(stocks_copy) - 1)
                    candidates = self._generate_placement_candidates(
                        stocks_copy[stock_idx], prod_size)
    ...
    def _generate_placement_candidates(self, stock, prod_size):
        ...
        if stock_w > prod_w and stock_h > prod_h:
            strategies.extend([
                ...
                (random.randint(0, stock_w - prod_w),
                 random.randint(0, stock_h - prod_h)),
    ...
    def geneticAlgo(self, observation, info):
        ...
        for generation in range(self.generations):
            ...
            if generation % 20 == 0:
                print(
                    f"Generation {generation}: Fitness = {global_best_fitness}")
    ...
    def get_action(self, observation, info):
        if self.policy_id == 1:
            ...
            sorted_stocks = sorted(
                enumerate(stocks),
                key=lambda s: self._get_stock_size_(
                    s[1])[0] * self._get_stock_size_(s[1])[1],
                reverse=True
            )
