...
class Policy2312981_2212168_2313063_2213561_2213381(Policy):
    def __init__(self, policy_id):
        ...
        if self.policy_id == 2:
            ...
            # Lưu trữ các stock đã hoàn thành (có bản cắt)
            self.used_stocks = []
...
# ================================================================
    def TwoSGP(self, observation):  # Two Stage General Patterns
        ...
        for stock_idx, stock in enumerate(stocks):
            ...
            # Mảng giả đại diện cho stock
            stock_grid = np.full((stock_width, stock_height), -1)
            current_pattern = []
            ...
            while strip_start < stock_width:
                # Tạo một dải mới với chiều rộng còn lại
                strip_width = stock_width - strip_start
                        ...
                        for i in range(strip_start, stock_width - product_width + 1):
                            ...
                            for j in range(stock_height - product_height + 1):
                                if quantity > 0 and self._can_place_(stock_grid, (i, j), (product_width, product_height)):
                                    ...
                                    stock_grid[i:i + product_width,
                                               j:j + product_height] = 1
            ...
            current_utilization = (
                used_area / (stock_width * stock_height)) * 100
...
# ================================================================
    def ThreeSHP(self, observation):
        ...
        products = sorted(products, key=lambda x: (
            x['quantity'], x['size'][0] * x['size'][1]), reverse=True)
        ...
        for stock_idx, stock in enumerate(stocks):  # Duyệt qua từng stock
            ...
            current_utilization = 0
            # Tạo mảng giả đại diện cho stock
            stock_grid = np.full((stock_width, stock_height), -1)
            for product in products:
            ...
            current_utilization = (
                total_area_used / (stock_width * stock_height)) * 100
...
#!================================================================
    ...
    def _policy_2(self, observation, info):
        ...
        if not self.is_best_plan:  # Nếu chưa có best_plan:
            ...
            if self.best_plan:
                print(
                    f"    |===]Best plan: The stock {self.best_plan[0]['stock_idx']} has {self.best_utimization:.5f}% utilization")
                for cut in self.best_plan:
                    print(
                        f"        |---]Stock_idx: {cut['stock_idx']}, size: {cut['size']}, position: {cut['position']}")

        # Kiểm tra best_plan có tồn tại không.
        if self.is_best_plan:
            # Lặp lại đến khi duyệt hết best_plan
            if self.best_plan_iterator < len(self.best_plan):
                # best plan này là cho toàn bộ hay cho 1 stock nhỉ
                cut = self.best_plan[self.best_plan_iterator]
                self.best_plan_iterator += 1
                return cut

            if self.best_plan_iterator == len(self.best_plan) and len(self.best_plan) != 0:
                # Lấy giá trị stock_idx từ phần tử đầu tiên của best_plan
                self.used_stocks.append(self.best_plan[0]["stock_idx"])
                self.best_plan = []
                self.is_best_plan = False
                self.best_utimization = 0
                self.best_plan_iterator = 0
