...
class Policy2352189_2352390_2352170_2352598_2352592(Policy):
    ...
    def new_individual(self):
        ...
        if np.random.random() < self.mutation_rate:
            ...
            # Dùng FFD
            indices = sorted(
                indices, key=lambda p: self.products[p]["size"][0] * self.products[p]["size"][1], reverse=True)
    ...
    def BestFitDecreasing(self, observation, info):
        if info["filled_ratio"] == 0:
            self.stocks = sorted(observation["stocks"], key=lambda p: (
                w := self._get_stock_size_(p))[0] * w[1])
    ...
    def heapifyStocks(self, index):
        ...
        while l < r:
            m = (l + r) // 2
            if np.count_nonzero(self.stocks[m] == -1) < np.count_nonzero(self.stocks[index] == -1):
                l = m + 1
            elif np.count_nonzero(self.stocks[m] == -1) == np.count_nonzero(self.stocks[index] == -1):
                r = m
            else:
                r = m - 1
