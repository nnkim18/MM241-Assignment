...
class Policy2352526_2352887_2353120_2353178_2353102(Policy):
    ...
    def sort_stock_sheet(self, stocks):
        ...
        sorted_stocks = sorted(stock_info_with_indices,
                               key=lambda x: x[0], reverse=True)

        self.stock_info_list = [(info[0], info[1], info[2])
    ...
    def get_action(self, observation, info):
        ...
        if self.policy_id == 1:
            if not self.first_action or info.get("filled_ratio") == 0.0:
                ...
                self.sorted_list_prods = self.SortProducts(
            ...
            for stock_index, stock_sheet in enumerate(self.stock_sheets):
                for product in self.sorted_list_prods:
                    if product["quantity"] > 0:
                        cut_position, product_size = self.cut_product(
                            product["size"], stock_sheet)

                        if cut_position == (-1, -1):
                            cut_position, product_size = self.cut_product(
                                product["size"][::-1], stock_sheet)
        ...
        if self.policy_id == 2:
            ...
            if not self.notUsed_stocks:
                ...
                self.sorted_products = [product for _, product in sorted(
                    products_list, key=lambda x: x[0], reverse=True)]
                stocks_with_index = []
                for i, stock in enumerate(observation['stocks']):
                    ...
                    stocks_with_index.append(
                        {'index': i, 'stock': stock, 'area': stock_area})
                self.notUsed_stocks = sorted(
                    stocks_with_index, key=lambda x: x['area'])

            for product in [p for p in self.sorted_products if p['quantity'] > 0]:
                prod_size = product['size']
                best_fit_stock_idx, best_fit_position, min_waste = None, None, float(
                    'inf')
        ...
        for row in range(num_rows - width + 1):
            for col in range(num_cols - height + 1):
                if self._can_place_(stock, (row, col), (width, height)) and not (row + width > num_rows) and not (col + height > num_cols):
                    current_waste = self.calculate_waste(
                        stock, row, col, width, height)
    ...
    def safe_get_stock_size(self, stock):
        try:
            ...
            if isinstance(stock_size, (list, np.ndarray)) and len(stock_size) >= 2:
                try:
                    size_0 = float(stock_size[0]) if isinstance(
                        stock_size[0], (int, float)) else 0
                    size_1 = float(stock_size[1]) if isinstance(
                        stock_size[1], (int, float)) else 0
