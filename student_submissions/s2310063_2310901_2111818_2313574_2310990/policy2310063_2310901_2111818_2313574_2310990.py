...
class local_search(Policy):
    ...
    def get_action(self, observation, info):
            ...
            for sol in more_sol:
                ...
                for _ in range(self.max_iterations):
                    ...
                    for neighbor in neighbors:
                        neighbor_objective = self.objective_function(
        ...
        if self.check:
            if self.current_index < len(self.solution):
                action = self.convert_to_action(
                    self.solution[self.current_index:self.current_index + 1], observation)
    ...
    def cal_prod(self, observation):
        ...
        for prod in list_prods:
            if prod["quantity"] > 0:
                ...
                area += prod["quantity"] * width * \
                    height  # Tính diện tích và cộng dồn
        return area
    ...
    def makeneighbor(self, current_solution, observation):
        ...
        stock_grids = [
            [[False] * self._get_stock_size_(stock)[0]
             for _ in range(self._get_stock_size_(stock)[1])]
            for stock in stocks
        ]
            ...
            if prod["quantity"] > 0:
                ...
                for _ in range(prod["quantity"]):  # Handle multiple quantities
                    ...
                    for stock_idx in unique_values:
                        ...
                        pos_x, pos_y, rotated = self.find_first_fit_position(
                            stock_grids[stock_idx], prod["size"])
                        if pos_x is not None and pos_y is not None:
                            if rotated:
                                # Cập nhật kích thước nếu xoay
                                prod_size = (prod["size"][1], prod["size"][0])
                                self._mark_grid_(
                                    stock_grids[stock_idx], (pos_x, pos_y), prod_size)
                                solution.append({
                                    "stock_idx": stock_idx,
                                    "size": prod_size,
                                ...
                                self._mark_grid_(
                                    stock_grids[stock_idx], (pos_x, pos_y), prod["size"])
                                solution.append({
                                    "stock_idx": stock_idx,
                                    "size": prod["size"],
                                    "position": (pos_x, pos_y)
    ...
    def generate_initial_solution(self, observation):
        ...
        stock_indices_sorted = list(range(len(stocks)))
        ...
        for prod in list_prods:
            if prod["quantity"] > 0:
                ...
                for _ in range(prod["quantity"]):  # Handle multiple quantities
                    ...
                    if not placed:
                        print(
                            f"Unable to place product {prod} due to lack of space.")
        return solution
...
class column(Policy):
    ...
    def calculate_remaining_space(self, stock_grid, prod_w, prod_h):
        ...
        total_space = len(stock_grid) * len(stock_grid[0])
        used_space = sum(row.count(True)
                         for row in stock_grid)  # Số ô đã được sử dụng
        product_space = prod_w * prod_h  # Diện tích sản phẩm
        # Trừ diện tích sản phẩm khỏi không gian trống
        remaining_space = total_space - used_space - product_space
    ...
    def solve_master_problem(self, stocks, products, patterns):
        ...
        b_eq = np.array([product["quantity"]
                        for product in products])  # Nhu cầu từng sản phẩm
        ...
        res = linprog(c, A_eq=A_eq, b_eq=b_eq, A_ub=A_stock, b_ub=b_ub, bounds=[
                      (0, None)] * num_patterns, method='highs')

        if res.success:
            # Lấy giá trị đối ngẫu (dual)
            # Hoặc kiểm tra 'upper', tùy thuộc vào bài toán
            dual_values = res.marginals['lower']
            return res.x, dual_values
        else:
            raise ValueError(
                f"Không tìm được lời giải cho Master Problem. Thông báo lỗi: {res.message}")
    ...
    def solve_subproblem(self, stocks, products, dual_values):
        ...
        for stock_index, (stock_w, stock_h) in enumerate(stock_sizes):
            ...
            for i, product in enumerate(products):
                ...
                A_ub.append(
                    [prod_w if j == i else 0 for j in range(num_products)])
                b_ub.append(stock_w)

                A_ub.append(
                    [prod_h if j == i else 0 for j in range(num_products)])
                b_ub.append(stock_h)

            res = linprog(c, A_ub=A_ub, b_ub=b_ub, bounds=[
                          (0, None)] * num_products, method='highs')
            ...
            if res.success:
                ...
                if reduced_cost < 0:
                    pattern = [stock_index] + \
                        list(map(lambda x: int(round(x)), res.x))
                    new_patterns.append(pattern)
            else:
                print(
                    f"Subproblem for stock {stock_index} failed to find a solution: {res.message}")
            ...
            for _ in range(prod["quantity"]):
                ...
                if selected_stock != -1 and pos_x is not None and pos_y is not None:
                    ...
                    if len(patterns) == 0 or selected_stock not in [pattern[0] for pattern in patterns]:
                        # Dòng đầu tiên là chỉ số stock
                        new_column = [selected_stock]
                        # Khởi tạo các giá trị 0 cho các sản phẩm khác
                        new_column.extend([0] * len(list_prods))
                        # Cập nhật số lượng sản phẩm loại k+1 vào cột mới
                        new_column[k + 1] = 1
                        patterns.append(new_column)
                    else:
                        column_index = next(i for i, pattern in enumerate(
                            patterns) if pattern[0] == selected_stock)
    ...
    def get_action(self, observation, info):
        ...
        if not self.check:
            ...
            self.patterns, self.solution = self.init_solution(
                list_stock, list_prods, stock_grids)
            while False:
                # Giải bài toán master để tìm số lần sử dụng mỗi pattern tối ưu
                usage, dual_values = self.solve_master_problem(
                    list_stock, list_prods, self.patterns)

            # Giải bài toán subproblem để tạo ra các patterns mới
                new_patterns = self.solve_subproblem(
                    list_stock, list_prods, dual_values)
